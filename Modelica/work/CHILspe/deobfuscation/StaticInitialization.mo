encapsulated package 'PSSDesign.eFMU4CHIL.Grid4CHILTesting: static initialization'
	...
	algorithm
		...
		succeeded := succeeded
		 and .DymolaCommands.SimulatorAPI.simulateModel(
		  problem = "'PSSDesign.eFMU4CHIL.Grid4CHILTesting: static initialization'.StartupModel",
		...
		if succeeded then
			...
			self.'B3.angle_0' := trajectories_buffer[9, 1];
			self.'B3.v_0' := trajectories_buffer[10, 1];
			...
			self.'G1.avr.s0' := trajectories_buffer[24, 1];
			self.'G1.avr.vref' := trajectories_buffer[25, 1];
			self.'derivative(G1.avr.vf1)' := trajectories_buffer[26, 1];
			self.'derivative(G1.avr.vm)' := trajectories_buffer[27, 1];
			self.'derivative(G1.avr.vr)' := trajectories_buffer[28, 1];
			self.'G1.avr.vf1' := trajectories_buffer[29, 1];
			self.'G1.avr.vm' := trajectories_buffer[30, 1];
			self.'G1.avr.vr' := trajectories_buffer[31, 1];
			self.'G1.machine.M' := trajectories_buffer[32, 1];
			self.'G1.machine.Sn' := trajectories_buffer[33, 1];
			self.'G1.machine.T1d0' := trajectories_buffer[34, 1];
			self.'G1.machine.T1q0' := trajectories_buffer[35, 1];
			self.'G1.machine.T2d0' := trajectories_buffer[36, 1];
			self.'G1.machine.T2q0' := trajectories_buffer[37, 1];
			self.'G1.machine.Taa' := trajectories_buffer[38, 1];
			self.'G1.machine.Vn' := trajectories_buffer[39, 1];
			self.'G1.machine.ra' := trajectories_buffer[40, 1];
			self.'G1.machine.x1d' := trajectories_buffer[41, 1];
			self.'G1.machine.x1q' := trajectories_buffer[42, 1];
			self.'G1.machine.x2d' := trajectories_buffer[43, 1];
			self.'G1.machine.x2q' := trajectories_buffer[44, 1];
			self.'G1.machine.xd' := trajectories_buffer[45, 1];
			self.'G1.machine.xq' := trajectories_buffer[46, 1];
			self.'G1.machine.I_MBtoSB' := trajectories_buffer[47, 1];
			self.'G1.machine.K1' := trajectories_buffer[48, 1];
			self.'G1.machine.K2' := trajectories_buffer[49, 1];
			self.'G1.machine.S_SBtoMB' := trajectories_buffer[50, 1];
			self.'G1.machine.V_MBtoSB' := trajectories_buffer[51, 1];
			self.'G1.machine.Z_MBtoSB' := trajectories_buffer[52, 1];
			self.'G1.machine.delta0' := trajectories_buffer[53, 1];
			self.'G1.machine.e1d0' := trajectories_buffer[54, 1];
			self.'G1.machine.e1q0' := trajectories_buffer[55, 1];
			self.'G1.machine.e2d0' := trajectories_buffer[56, 1];
			self.'G1.machine.e2q0' := trajectories_buffer[57, 1];
			self.'G1.machine.id0' := trajectories_buffer[58, 1];
			self.'G1.machine.iq0' := trajectories_buffer[59, 1];
			self.'G1.machine.p0' := trajectories_buffer[60, 1];
			self.'G1.machine.pm00' := trajectories_buffer[61, 1];
			self.'G1.machine.q0' := trajectories_buffer[62, 1];
			self.'G1.machine.vd0' := trajectories_buffer[63, 1];
			self.'G1.machine.vf00' := trajectories_buffer[64, 1];

			trajectories_buffer[1:52] := .DymolaCommands.Trajectories.readTrajectory(
			 fileName = result_file + ".mat",
			 rows = 1,
			 signals = {
			  "self.'G1.machine.vq0'",
			...
			self.'G1.machine.vq0' := trajectories_buffer[1, 1];
			self.'G1.machine.w_b' := trajectories_buffer[2, 1];
			self.'derivative(G1.machine.delta)' := trajectories_buffer[3, 1];
			self.'derivative(G1.machine.e1d)' := trajectories_buffer[4, 1];
			self.'derivative(G1.machine.e1q)' := trajectories_buffer[5, 1];
			self.'derivative(G1.machine.e2d)' := trajectories_buffer[6, 1];
			self.'derivative(G1.machine.e2q)' := trajectories_buffer[7, 1];
			self.'derivative(G1.machine.w)' := trajectories_buffer[8, 1];
			self.'G1.machine.delta' := trajectories_buffer[9, 1];
			self.'G1.machine.e1d' := trajectories_buffer[10, 1];
			self.'G1.machine.e1q' := trajectories_buffer[11, 1];
			self.'G1.machine.e2d' := trajectories_buffer[12, 1];
			self.'G1.machine.e2q' := trajectories_buffer[13, 1];
			self.'G1.machine.id' := trajectories_buffer[14, 1];
			self.'G1.machine.iq' := trajectories_buffer[15, 1];
			self.'G1.machine.w' := trajectories_buffer[16, 1];
			self.'G1.machine.I0.im' := trajectories_buffer[17, 1];
			self.'G1.machine.I0.re' := trajectories_buffer[18, 1];
			self.'G1.machine.Idq0.im' := trajectories_buffer[19, 1];
			self.'G1.machine.Idq0.re' := trajectories_buffer[20, 1];
			self.'G1.machine.S0.im' := trajectories_buffer[21, 1];
			self.'G1.machine.Vdq0.im' := trajectories_buffer[22, 1];
			self.'G1.machine.Vdq0.re' := trajectories_buffer[23, 1];
			self.'G1.machine.Vt0.im' := trajectories_buffer[24, 1];
			self.'G1.machine.Vt0.re' := trajectories_buffer[25, 1];
			self.'G1.machine.auxCMval.im' := trajectories_buffer[26, 1];
			self.'G1.machine.auxCMval.re' := trajectories_buffer[27, 1];
			self.'L1.B' := trajectories_buffer[28, 1];
			self.'L1.G' := trajectories_buffer[29, 1];
			self.'L1.R' := trajectories_buffer[30, 1];
			self.'L1.X' := trajectories_buffer[31, 1];
			self.'L2.B' := trajectories_buffer[32, 1];
			self.'L2.G' := trajectories_buffer[33, 1];
			self.'L2.R' := trajectories_buffer[34, 1];
			self.'L2.X' := trajectories_buffer[35, 1];
			self.'L2.p.ii' := trajectories_buffer[36, 1];
			self.'L2.p.ir' := trajectories_buffer[37, 1];
			self.'SysData.S_b' := trajectories_buffer[38, 1];
			self.'SysData.fn' := trajectories_buffer[39, 1];
			self.'transformer.Sn' := trajectories_buffer[40, 1];
			self.'transformer.V_b' := trajectories_buffer[41, 1];
			self.'transformer.Vn' := trajectories_buffer[42, 1];
			self.'transformer.m' := trajectories_buffer[43, 1];
			self.'transformer.rT' := trajectories_buffer[44, 1];
			self.'transformer.xT' := trajectories_buffer[45, 1];
			self.'transformer.Zb' := trajectories_buffer[46, 1];
			self.'transformer.Zn' := trajectories_buffer[47, 1];
			self.'transformer.r' := trajectories_buffer[48, 1];
			self.'transformer.x' := trajectories_buffer[49, 1];
			self.'transformer.n.ii' := trajectories_buffer[50, 1];
			self.'transformer.n.ir' := trajectories_buffer[51, 1];
			self.'discrete.stepSize' := trajectories_buffer[52, 1];
		...
		assert(succeeded, "Simulation of 'PSSDesign.eFMU4CHIL.Grid4CHILTesting: static initialization'.StartupModel failed.");
...
if false then
		...
		self.'G1.avr.vr' := ((self.K0 * (1.0 - (self.'G1.avr.T1' / self.'G1.avr.T2'))) * ((self.'G1.avr.vref' + self.vf) - self.'G1.avr.vm'));
	end Reinitialize;
...
end 'PSSDesign.eFMU4CHIL.Grid4CHILTesting: static initialization';
