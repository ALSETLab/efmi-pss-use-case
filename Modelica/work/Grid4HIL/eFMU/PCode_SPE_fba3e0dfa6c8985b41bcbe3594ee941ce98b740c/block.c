...
#define ALGONAME    "Hb0cb05f437863e9fece1a90bb931dbfeb4ac00f3_fba3e0dfa6c8985b41bcbe3594ee941ce98b740c"
...
static void Recalibrate(ALGOSTRUCT *instance)
{
    ...
    */
    ...
    /* (255:3) self.'transformer.Zn' := (self.'transformer.Vn' ^ 2) / self.'transformer.Sn'; */
    instance->transformer_Zn = (ESP_square(instance->transformer_Vn)) / instance->transformer_Sn;

    /* (256:3) self.'transformer.Zb' := (self.'transformer.V_b' ^ 2) / self.'SysData.S_b'; */
    instance->transformer_Zb = (ESP_square(instance->transformer_V_b)) / instance->SysData_S_b;

    /* (257:3) self.'transformer.r' := (self.'transformer.rT' * self.'transformer.Zn') / self.'transformer.Zb'; */
    instance->transformer_r = (instance->transformer_rT * instance->transformer_Zn) / instance->transformer_Zb;

    /* (258:3) self.'transformer.x' := (self.'transformer.xT' * self.'transformer.Zn') / self.'transformer.Zb'; */
    instance->transformer_x = (instance->transformer_xT * instance->transformer_Zn) / instance->transformer_Zb;

    /* (259:3) self.'G1.machine.w_b' := 6.28318530717958623 * self.'SysData.fn'; */
    instance->G1_machine_w_b = 6.283185307179586 * instance->SysData_fn;

    /* (260:3) self.'G1.machine.S_SBtoMB' := self.'SysData.S_b' / self.'G1.machine.Sn'; */
    instance->G1_machine_S_SBtoMB = instance->SysData_S_b / instance->G1_machine_Sn;

    /* (261:3) self.'G1.machine.I_MBtoSB' := (self.'G1.machine.Sn' * self.'G1.V_b') / (self.'SysData.S_b' * self.'G1.machine.Vn'); */
    instance->G1_machine_I_MBtoSB = (instance->G1_machine_Sn * instance->G1_V_b) / (instance->SysData_S_b * instance->G1_machine_Vn);

    /* (262:3) self.'G1.machine.V_MBtoSB' := self.'G1.machine.Vn' / self.'G1.V_b'; */
    instance->G1_machine_V_MBtoSB = instance->G1_machine_Vn / instance->G1_V_b;

    /* (263:3) self.'G1.machine.Z_MBtoSB' := (self.'SysData.S_b' * (self.'G1.machine.Vn' ^ 2)) / (self.'G1.machine.Sn' * (self.'G1.V_b' ^ 2)); */
    ...
    /* (264:3) self.'G1.machine.p0' := self.'G1.P_0' / self.'SysData.S_b'; */
    instance->G1_machine_p0 = instance->G1_P_0 / instance->SysData_S_b;

    /* (265:3) self.'G1.machine.q0' := self.'G1.Q_0' / self.'SysData.S_b'; */
    instance->G1_machine_q0 = instance->G1_Q_0 / instance->SysData_S_b;

    /* (266:3) self.'G1.machine.Vt0.re' := self.'G1.v_0' * cos(self.'G1.angle_0'); */
    instance->G1_machine_Vt0_re = instance->G1_v_0 * ESP_cos(instance->G1_angle_0);

    /* (267:3) self.'G1.machine.Vt0.im' := self.'G1.v_0' * sin(self.'G1.angle_0'); */
    instance->G1_machine_Vt0_im = instance->G1_v_0 * ESP_sin(instance->G1_angle_0);

    /* (268:3) self.'G1.machine.S0.im' := (-1.0) * self.'G1.machine.q0'; */
    instance->G1_machine_S0_im = (-1.0) * instance->G1_machine_q0;

    /* (269:3) self.'G1.machine.I0.re' := ((self.'G1.machine.p0' * self.'G1.machine.Vt0.re') - (self.'G1.machine.S0.im' * self.'G1.machine.Vt0.im')) / ((self.'G1.machine.Vt0.re' * self.'G1.machine.Vt0.re') + (self.'G1.machine.Vt0.im' * self.'G1.machine.Vt0.im')); */
    ...
    /* (270:3) self.'G1.machine.I0.im' := ((self.'G1.machine.p0' * self.'G1.machine.Vt0.im') + (self.'G1.machine.S0.im' * self.'G1.machine.Vt0.re')) / ((self.'G1.machine.Vt0.re' * self.'G1.machine.Vt0.re') + (self.'G1.machine.Vt0.im' * self.'G1.machine.Vt0.im')); */
    ...
    /* (271:3) self.'G1.machine.auxCMval.re' := self.'G1.machine.Vt0.re' + (((self.'G1.machine.ra' * self.'G1.machine.Z_MBtoSB') * self.'G1.machine.I0.re') - ((self.'G1.machine.xq' * self.'G1.machine.Z_MBtoSB') * self.'G1.machine.I0.im')); */
    ...
    /* (272:3) self.'G1.machine.auxCMval.im' := self.'G1.machine.Vt0.im' + (((self.'G1.machine.ra' * self.'G1.machine.Z_MBtoSB') * self.'G1.machine.I0.im') + ((self.'G1.machine.xq' * self.'G1.machine.Z_MBtoSB') * self.'G1.machine.I0.re')); */
    ...
    /* (273:3) self.'G1.machine.delta0' := atan2(self.'G1.machine.auxCMval.im', self.'G1.machine.auxCMval.re'); */
    instance->G1_machine_delta0 = ESP_atan2(instance->G1_machine_auxCMval_im, instance->G1_machine_auxCMval_re);

    /* (274:3) self.'G1.machine.Vdq0.re' := ((self.'G1.machine.Vt0.re' * cos(1.57079632679489656 - self.'G1.machine.delta0')) - (self.'G1.machine.Vt0.im' * sin(1.57079632679489656 - self.'G1.machine.delta0'))) / self.'G1.machine.V_MBtoSB'; */
    instance->G1_machine_Vdq0_re = ((instance->G1_machine_Vt0_re * ESP_cos(1.5707963267948966 - instance->G1_machine_delta0)) \
        - (instance->G1_machine_Vt0_im * ESP_sin(1.5707963267948966 - instance->G1_machine_delta0))) / instance->G1_machine_V_MBtoSB;

    /* (275:3) self.'G1.machine.Vdq0.im' := ((self.'G1.machine.Vt0.re' * sin(1.57079632679489656 - self.'G1.machine.delta0')) + (self.'G1.machine.Vt0.im' * cos(1.57079632679489656 - self.'G1.machine.delta0'))) / self.'G1.machine.V_MBtoSB'; */
    instance->G1_machine_Vdq0_im = ((instance->G1_machine_Vt0_re * ESP_sin(1.5707963267948966 - instance->G1_machine_delta0)) \
        + (instance->G1_machine_Vt0_im * ESP_cos(1.5707963267948966 - instance->G1_machine_delta0))) / instance->G1_machine_V_MBtoSB;

    /* (276:3) self.'G1.machine.Idq0.re' := ((self.'G1.machine.I0.re' * cos(1.57079632679489656 - self.'G1.machine.delta0')) - (self.'G1.machine.I0.im' * sin(1.57079632679489656 - self.'G1.machine.delta0'))) / self.'G1.machine.I_MBtoSB'; */
    instance->G1_machine_Idq0_re = ((instance->G1_machine_I0_re * ESP_cos(1.5707963267948966 - instance->G1_machine_delta0)) \
        - (instance->G1_machine_I0_im * ESP_sin(1.5707963267948966 - instance->G1_machine_delta0))) / instance->G1_machine_I_MBtoSB;

    /* (277:3) self.'G1.machine.Idq0.im' := ((self.'G1.machine.I0.re' * sin(1.57079632679489656 - self.'G1.machine.delta0')) + (self.'G1.machine.I0.im' * cos(1.57079632679489656 - self.'G1.machine.delta0'))) / self.'G1.machine.I_MBtoSB'; */
    instance->G1_machine_Idq0_im = ((instance->G1_machine_I0_re * ESP_sin(1.5707963267948966 - instance->G1_machine_delta0)) \
        + (instance->G1_machine_I0_im * ESP_cos(1.5707963267948966 - instance->G1_machine_delta0))) / instance->G1_machine_I_MBtoSB;

    /* (278:3) self.'G1.machine.vd0' := self.'G1.machine.Vdq0.re'; */
    instance->G1_machine_vd0 = instance->G1_machine_Vdq0_re;

    /* (279:3) self.'G1.machine.vq0' := self.'G1.machine.Vdq0.im'; */
    instance->G1_machine_vq0 = instance->G1_machine_Vdq0_im;

    /* (280:3) self.'G1.machine.id0' := self.'G1.machine.Idq0.re'; */
    instance->G1_machine_id0 = instance->G1_machine_Idq0_re;

    /* (281:3) self.'G1.machine.iq0' := self.'G1.machine.Idq0.im'; */
    instance->G1_machine_iq0 = instance->G1_machine_Idq0_im;

    /* (282:3) self.'G1.machine.pm00' := (((self.'G1.machine.vq0' + (self.'G1.machine.ra' * self.'G1.machine.iq0')) * self.'G1.machine.iq0') + ((self.'G1.machine.vd0' + (self.'G1.machine.ra' * self.'G1.machine.id0')) * self.'G1.machine.id0')) / self.'G1.machine.S_SBtoMB'; */
    ...
    /* (283:3) self.'G1.machine.e2q0' := (self.'G1.machine.vq0' + (self.'G1.machine.ra' * self.'G1.machine.iq0')) + (self.'G1.machine.x2d' * self.'G1.machine.id0'); */
    ...
    /* (284:3) self.'G1.machine.e2d0' := (self.'G1.machine.vd0' + (self.'G1.machine.ra' * self.'G1.machine.id0')) - (self.'G1.machine.x2q' * self.'G1.machine.iq0'); */
    ...
    /* (285:3) self.'G1.machine.e1d0' := ((self.'G1.machine.xq' - self.'G1.machine.x1q') - (((self.'G1.machine.T2q0' * self.'G1.machine.x2q') * (self.'G1.machine.xq' - self.'G1.machine.x1q')) / (self.'G1.machine.T1q0' * self.'G1.machine.x1q'))) * self.'G1.machine.iq0'; */
    ...
    /* (286:3) self.'G1.machine.K1' := (self.'G1.machine.xd' - self.'G1.machine.x1d') - (((self.'G1.machine.T2d0' * self.'G1.machine.x2d') * (self.'G1.machine.xd' - self.'G1.machine.x1d')) / (self.'G1.machine.T1d0' * self.'G1.machine.x1d')); */
    ...
    /* (287:3) self.'G1.machine.K2' := (self.'G1.machine.x1d' - self.'G1.machine.x2d') + (((self.'G1.machine.T2d0' * self.'G1.machine.x2d') * (self.'G1.machine.xd' - self.'G1.machine.x1d')) / (self.'G1.machine.T1d0' * self.'G1.machine.x1d')); */
    instance->G1_machine_K2 = (instance->G1_machine_x1d - instance->G1_machine_x2d) + ((instance->G1_machine_T2d0 * instance->G1_machine_x2d \
        * (instance->G1_machine_xd - instance->G1_machine_x1d)) / (instance->G1_machine_T1d0 * instance->G1_machine_x1d));

    /* (288:3) self.'G1.machine.e1q0' := (self.'G1.machine.e2q0' + (self.'G1.machine.K2' * self.'G1.machine.id0')) - ((self.'G1.machine.Taa' * (((self.'G1.machine.K1' + self.'G1.machine.K2') * self.'G1.machine.id0') + self.'G1.machine.e2q0')) / self.'G1.machine.T1d0'); */
    ...
    /* (289:3) self.'G1.machine.vf00' := (self.'G1.machine.V_MBtoSB' * ((self.'G1.machine.K1' * self.'G1.machine.id0') + self.'G1.machine.e1q0')) / (1.0 - (self.'G1.machine.Taa' / self.'G1.machine.T1d0')); */
    ...
    /* (290:3) self.'G1.feedbackVf0.u2' := self.'G1.gainVf0.k' * self.'G1.machine.vf00'; */
    instance->G1_feedbackVf0_u2 = instance->G1_gainVf0_k * instance->G1_machine_vf00;

    /* (291:3) self.vf0OUT := self.'G1.machine.vf00'; */
...
static void Reinitialize(ALGOSTRUCT *instance)
{
    ...
    */
    ...
    /* (177:3) self.'derivative(G1.machine.e1q)' := 0.0; */
    instance->der_G1_machine_e1q = 0.0;

    /* (178:3) self.'derivative(G1.machine.e2q)' := 0.0; */
    instance->der_G1_machine_e2q = 0.0;

    /* (179:3) self.'derivative(G1.machine.w)' := 0.0; */
    instance->der_G1_machine_w = 0.0;

    /* (180:3) self.'derivative(G1.machine.delta)' := 0.0; */
    ...
    */
    ...
    _A0[4] = ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m \
        ...
        + ((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / instance->transformer_m) \
        - (1.0 / (ESP_square(instance->transformer_m)))) * (instance->G1_machine_T2q0 * ((instance->G1_machine_x1q - instance->G1_machine_x2q) \
        ...
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) \
        * instance->G1_machine_x2q)) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m \
        / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_ra) - (instance->transformer_x \
        * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta)))) + (instance->transformer_r * (instance->G1_machine_I_MBtoSB \
        * ESP_cos(instance->G1_machine_delta)));
    _A0[5] = ((instance->transformer_x * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta))) - ((instance->G1_machine_V_MBtoSB \
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) \
        * instance->G1_machine_ra) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m \
        / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_x2d))) + (instance->transformer_r \
        * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta)));
    _A0[6] = instance->transformer_r / instance->transformer_m;
    _A0[7] = instance->transformer_x / instance->transformer_m;
    _A0[8] = 0.0;
    _A0[9] = 0.0;
    _A0[10] = ((instance->transformer_r * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta))) - (((instance->G1_machine_V_MBtoSB \
        * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) \
        ...
        * instance->G1_machine_x2q * (instance->G1_machine_xq - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 \
        / instance->G1_machine_x1q)))) / instance->G1_machine_T1q0) + ((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * \
        (instance->G1_machine_T2q0 * ((instance->G1_machine_x1q - instance->G1_machine_x2q) + ((instance->G1_machine_T2q0 \
        * instance->G1_machine_x2q * (instance->G1_machine_xq - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 \
        / instance->G1_machine_x1q)))) / instance->G1_machine_T2q0) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * \
        instance->G1_machine_x2q) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m \
        / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_ra))) + (instance->transformer_x \
        * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta)));
    _A0[11] = ((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / \
        instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_ra) - (instance->G1_machine_V_MBtoSB \
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) \
        * instance->G1_machine_x2d) - (instance->transformer_r * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta)))) \
        + (instance->transformer_x * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta)));
    _A0[12] = ((((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G))) * instance->transformer_r) - ((1.0 + ((instance->L1_R * instance->L1_G) - \
        (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X))) \
        * instance->transformer_x) - instance->L1_X;
    _A0[13] = ((1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X))) * instance->transformer_r) + ((((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))) \
        * instance->transformer_x) + instance->L1_R;
    _A0[14] = instance->L1_R;
    _A0[15] = (-1.0) * instance->L1_X;
    _A0[16] = (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) \
        ...
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) \
        ...
        * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G))))) * (instance->G1_machine_T2q0 * ((instance->G1_machine_x1q - instance->G1_machine_x2q) \
        + ((instance->G1_machine_T2q0 * instance->G1_machine_x2q * (instance->G1_machine_xq - instance->G1_machine_x1q)) \
        / instance->G1_machine_T1q0 / instance->G1_machine_x1q)))) / instance->transformer_m / instance->G1_machine_T2q0) \
        + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) \
        - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) \
        * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) * instance->G1_machine_x2q) \
        / instance->transformer_m)) - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + \
        ((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_X)))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) \
        * instance->G1_machine_ra) / instance->transformer_m);
    _A0[17] = (-1.0) * (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_X)))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) \
        * instance->G1_machine_ra) + (((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + ((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_X)))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) \
        * instance->G1_machine_x2d)) / instance->transformer_m);
    _A0[18] = (((1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))) * instance->transformer_r) - (((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R)) \
        * instance->transformer_x)) + instance->L1_R;
    _A0[19] = (((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R)) * instance->transformer_r) + ((1.0 + (((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))) \
        * instance->transformer_x) + instance->L1_X;
    _A0[20] = instance->L1_X;
    _A0[21] = instance->L1_R;
    _A0[22] = (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) \
        - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + (((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) \
        * (instance->G1_machine_T1q0 * (instance->G1_machine_xq - instance->G1_machine_x1q - ((instance->G1_machine_T2q0 \
        * instance->G1_machine_x2q * (instance->G1_machine_xq - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 \
        ...
        * ESP_sin(instance->G1_machine_delta) * ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) \
        + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) - (instance->G1_machine_V_MBtoSB \
        * ESP_cos(instance->G1_machine_delta) * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) \
        + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) * (instance->G1_machine_T2q0 \
        ...
        * ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + (((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) \
        * instance->L1_X))))) * instance->G1_machine_x2q) / instance->transformer_m)) - ((((instance->G1_machine_V_MBtoSB \
        * ESP_cos(instance->G1_machine_delta) * ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) \
        + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) + (instance->G1_machine_V_MBtoSB \
        * ESP_sin(instance->G1_machine_delta) * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) \
        + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) * instance->G1_machine_ra) / \
        instance->transformer_m);
    _A0[23] = (-1.0) * (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) \
        - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + (((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) \
        * instance->G1_machine_ra) + (((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((instance->L1_R \
        * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + (((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) \
        * instance->L1_X))))) * instance->G1_machine_x2d)) / instance->transformer_m);
    _A0[24] = (-1.0) * (((1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))) * instance->transformer_x) \
        + (((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)) * instance->transformer_r));
    _A0[25] = ((1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))) * instance->transformer_r) \
        - (((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)) * instance->transformer_x);
    _A0[26] = (-1.0) * instance->L2_R;
    _A0[27] = instance->L2_X;
    _A0[28] = (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L2_R * instance->L2_G) \
        - (instance->L2_X * instance->L2_B)))) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * \
        ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)))) * (instance->G1_machine_T1q0 * (instance->G1_machine_xq \
        - instance->G1_machine_x1q - ((instance->G1_machine_T2q0 * instance->G1_machine_x2q * (instance->G1_machine_xq \
        - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 / instance->G1_machine_x1q)))) / instance->transformer_m \
        / instance->G1_machine_T1q0) + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + \
        ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B)))) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)))) * (instance->G1_machine_T2q0 * ((instance->G1_machine_x1q \
        - instance->G1_machine_x2q) + ((instance->G1_machine_T2q0 * instance->G1_machine_x2q * (instance->G1_machine_xq \
        - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 / instance->G1_machine_x1q)))) / instance->transformer_m \
        / instance->G1_machine_T2q0) + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + \
        ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B)))) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)))) * instance->G1_machine_x2q) / instance->transformer_m)) \
        - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (1.0 + ((instance->L2_R * instance->L2_G) \
        - (instance->L2_X * instance->L2_B)))) - (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * \
        ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)))) * instance->G1_machine_ra) / instance->transformer_m);
    _A0[29] = (-1.0) * (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L2_R \
        * instance->L2_G) - (instance->L2_X * instance->L2_B)))) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)))) * instance->G1_machine_ra) + (((instance->G1_machine_V_MBtoSB \
        * ESP_cos(instance->G1_machine_delta) * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B)))) \
        - (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((instance->L2_R * instance->L2_B) + (instance->L2_X \
        * instance->L2_G)))) * instance->G1_machine_x2d)) / instance->transformer_m);
    _A0[30] = ((1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))) * instance->transformer_r) \
        - (((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)) * instance->transformer_x);
    _A0[31] = (((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G)) * instance->transformer_r) + ((1.0 \
        + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))) * instance->transformer_x);
    _A0[32] = (-1.0) * instance->L2_X;
    _A0[33] = (-1.0) * instance->L2_R;
    _A0[34] = (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((instance->L2_R * instance->L2_B) \
        + (instance->L2_X * instance->L2_G))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * \
        (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * (instance->G1_machine_T1q0 \
        * (instance->G1_machine_xq - instance->G1_machine_x1q - ((instance->G1_machine_T2q0 * instance->G1_machine_x2q \
        * (instance->G1_machine_xq - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 / instance->G1_machine_x1q)))) \
        / instance->transformer_m / instance->G1_machine_T1q0) + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * (instance->G1_machine_T2q0 \
        * ((instance->G1_machine_x1q - instance->G1_machine_x2q) + ((instance->G1_machine_T2q0 * instance->G1_machine_x2q \
        * (instance->G1_machine_xq - instance->G1_machine_x1q)) / instance->G1_machine_T1q0 / instance->G1_machine_x1q)))) \
        / instance->transformer_m / instance->G1_machine_T2q0) + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * instance->G1_machine_x2q) \
        / instance->transformer_m)) - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((instance->L2_R \
        * instance->L2_B) + (instance->L2_X * instance->L2_G))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * instance->G1_machine_ra) \
        / instance->transformer_m);
    _A0[35] = (-1.0) * (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((instance->L2_R * instance->L2_B) \
        + (instance->L2_X * instance->L2_G))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * \
        (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * instance->G1_machine_ra) + \
        (((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((instance->L2_R * instance->L2_B) + (instance->L2_X \
        * instance->L2_G))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + ((instance->L2_R \
        * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * instance->G1_machine_x2d)) / instance->transformer_m);
    solution_buffer_for_x_6[0] = (-1.0) * (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 \
        / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2q);
    solution_buffer_for_x_6[1] = (-1.0) * (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 \
        / instance->transformer_m / instance->transformer_m) - (1.0 / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2q);
    solution_buffer_for_x_6[2] = instance->B3_p_vr - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * (((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) \
        + (instance->L1_X * instance->L1_G))))) * instance->G1_machine_e2q) / instance->transformer_m);
    solution_buffer_for_x_6[3] = instance->B3_p_vi - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (1.0 + (((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) \
        * instance->L1_X))))) * instance->G1_machine_e2q) / instance->transformer_m);
    solution_buffer_for_x_6[4] = instance->B3_p_vr - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B)))) - (instance->G1_machine_V_MBtoSB \
    ...
    solution_buffer_for_x_6[5] = instance->B3_p_vi - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) * instance->G1_machine_e2q) \
        / instance->transformer_m);
    ...
    /* (221:3) if signal in SOLVE_LINEAR_EQUATIONS_FAILED then... */
    ...
    if(ESP_tmp_signals != ErrorSignal_NONE) {
        ...
        /* (223:4) self.'transformer.n.ii' := 1.00000000000000008e-15; */
        instance->transformer_n_ii = 1.0E-15;

        /* (224:4) self.'transformer.n.ir' := 1.00000000000000008e-15; */
        instance->transformer_n_ir = 1.0E-15;

        /* (225:4) self.'L2.p.ir' := 1.00000000000000008e-15; */
        instance->L2_p_ir = 1.0E-15;

        /* (226:4) self.'L2.p.ii' := 1.00000000000000008e-15; */
        instance->L2_p_ii = 1.0E-15;

        /* (227:4) self.'G1.machine.iq' := self.'G1.machine.iq0'; */
        instance->G1_machine_iq = instance->G1_machine_iq0;

        /* (228:4) self.'G1.machine.id' := self.'G1.machine.id0'; */
        instance->G1_machine_id = instance->G1_machine_id0;

        /* (229:4) signal SOLVE_LINEAR_EQUATIONS_FAILED */
        instance->ErrorSignals |= ErrorSignal_SOLVE_LINEAR_EQUATIONS_FAILED;
    } else {
        /* Only if the system can be solved, assign computed states: */

        /* (232:4) self.'transformer.n.ii' := 'solution_buffer.for.x[6]'[1]; */
        instance->transformer_n_ii = solution_buffer_for_x_6[0];

        /* (233:4) self.'transformer.n.ir' := 'solution_buffer.for.x[6]'[2]; */
        instance->transformer_n_ir = solution_buffer_for_x_6[1];

        /* (234:4) self.'L2.p.ir' := 'solution_buffer.for.x[6]'[3]; */
        instance->L2_p_ir = solution_buffer_for_x_6[2];

        /* (235:4) self.'L2.p.ii' := 'solution_buffer.for.x[6]'[4]; */
        instance->L2_p_ii = solution_buffer_for_x_6[3];

        /* (236:4) self.'G1.machine.iq' := 'solution_buffer.for.x[6]'[5]; */
        instance->G1_machine_iq = solution_buffer_for_x_6[4];

        /* (237:4) self.'G1.machine.id' := 'solution_buffer.for.x[6]'[6]; */
...
static void Startup(ALGOSTRUCT *instance)
{
    ...
    */
    ...
    /* (306:3) self.'discrete.stepSize' := 1.00000000000000002e-3; */
    instance->discrete_stepSize = 0.001;

    /* (307:3) self.'G1.V_b' := 4.0e+5; */
    instance->G1_V_b = 400000.0;

    /* (308:3) self.'G1.machine.Vn' := 4.0e+5; */
    instance->G1_machine_Vn = 400000.0;

    /* (309:3) self.'transformer.m' := 1.0; */
    instance->transformer_m = 1.0;

    /* (310:3) self.'G1.machine.x2q' := 2.5e-1; */
    instance->G1_machine_x2q = 0.25;

    /* (311:3) self.'G1.machine.ra' := 3.00000000000000006e-3; */
    instance->G1_machine_ra = 0.003;

    /* (312:3) self.'G1.machine.x2d' := 2.3000000000000001e-1; */
    instance->G1_machine_x2d = 0.23;

    /* (313:3) self.'L1.X' := 2.25225225225225215e-2; */
    instance->L1_X = 0.02252252252252252;
    ...
    /* (318:3) self.'L2.X' := 4.18918918918918942e-2; */
    instance->L2_X = 0.041891891891891894;
    ...
    /* (321:3) self.'G1.machine.Taa' := 2.00000000000000004e-3; */
    instance->G1_machine_Taa = 0.002;

    /* (322:3) self.'G1.machine.T1d0' := 8.0; */
    instance->G1_machine_T1d0 = 8.0;

    /* (323:3) self.'G1.machine.xd' := 1.81000000000000005; */
    instance->G1_machine_xd = 1.81;

    /* (324:3) self.'G1.machine.x1d' := 2.99999999999999989e-1; */
    instance->G1_machine_x1d = 0.3;

    /* (325:3) self.'G1.machine.T2d0' := 2.99999999999999989e-2; */
    instance->G1_machine_T2d0 = 0.03;

    /* (326:3) self.'G1.machine.M' := 7.0; */
    instance->G1_machine_M = 7.0;

    /* (327:3) self.'G1.machine.xq' := 1.76000000000000001; */
    instance->G1_machine_xq = 1.76;

    /* (328:3) self.'G1.machine.x1q' := 6.50000000000000022e-1; */
    instance->G1_machine_x1q = 0.65;

    /* (329:3) self.'G1.machine.T2q0' := 7.00000000000000067e-2; */
    instance->G1_machine_T2q0 = 0.07;

    /* (330:3) self.'G1.machine.T1q0' := 1.0; */
    instance->G1_machine_T1q0 = 1.0;

    /* (331:3) self.'SysData.S_b' := 1.0e+8; */
    instance->SysData_S_b = 1.0E8;

    /* (332:3) self.'B3.v_0' := 1.0; */
    instance->B3_v_0 = 1.0;

    /* (333:3) self.'B3.angle_0' := 0.0; */
    instance->B3_angle_0 = 0.0;

    /* (334:3) self.'G1.gainVf0.k' := -1.0; */
    instance->G1_gainVf0_k = -1.0;

    /* (335:3) self.'G1.v_0' := 1.0; */
    instance->G1_v_0 = 1.0;

    /* (336:3) self.'G1.angle_0' := 4.94677176989159972e-1; */
    instance->G1_angle_0 = 0.49467717698916;

    /* (337:3) self.'G1.Q_0' := 9.67924969906579971e+8; */
    instance->G1_Q_0 = 9.6792496990658E8;

    /* (338:3) self.'G1.P_0' := 1.99799999999359989e+9; */
    instance->G1_P_0 = 1.9979999999936E9;

    /* (339:3) self.'G1.machine.Sn' := 2.22e+9; */
    instance->G1_machine_Sn = 2.22E9;

    /* (340:3) self.'SysData.fn' := 6.0e+1; */
    instance->SysData_fn = 60.0;

    /* (341:3) self.'transformer.xT' := 1.49999999999999994e-1; */
    instance->transformer_xT = 0.15;

    /* (342:3) self.'transformer.rT' := 0.0; */
    instance->transformer_rT = 0.0;

    /* (343:3) self.'transformer.V_b' := 4.0e+5; */
    instance->transformer_V_b = 400000.0;

    /* (344:3) self.'transformer.Vn' := 4.0e+5; */
    instance->transformer_Vn = 400000.0;

    /* (345:3) self.'transformer.Sn' := 2.22e+9; */
    instance->transformer_Sn = 2.22E9;
...
static void DoStep(ALGOSTRUCT *instance)
{
    ...
    _A3[4] = (((instance->transformer_m * instance->transformer_r * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta))) \
        ...
        + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) \
        * instance->transformer_m) / (ESP_square(instance->transformer_m)))) * instance->G1_machine_x2q)) - (instance->G1_machine_V_MBtoSB \
        * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) * instance->transformer_m) \
    ...
    _A3[10] = ((instance->transformer_m * instance->transformer_x * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta))) \
        ...
        - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) \
        * instance->transformer_m) / (ESP_square(instance->transformer_m)))) * instance->G1_machine_x2q) - (instance->G1_machine_V_MBtoSB \
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) * instance->transformer_m) \
        / (ESP_square(instance->transformer_m)))) * instance->G1_machine_ra);
    _A3[11] = (((instance->transformer_m * instance->transformer_x * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta))) \
        ...
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) * instance->transformer_m) \
        / (ESP_square(instance->transformer_m)))) * instance->G1_machine_x2d);
    _A3[12] = (-1.0) * instance->L1_X;
    _A3[13] = instance->L1_R;
    _A3[14] = instance->L1_R;
    _A3[15] = (-1.0) * instance->L1_X;
    _A3[16] = (((((instance->transformer_m * instance->transformer_x * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X \
        * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) - (instance->transformer_m \
        * instance->transformer_r * (((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 \
        : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) \
        - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) * (instance->G1_machine_I_MBtoSB * \
        ESP_sin(instance->G1_machine_delta))) - (((instance->transformer_m * instance->transformer_r * (1.0 + ((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_X)))) + (instance->transformer_m * instance->transformer_x * (((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) \
        * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta)))) + ((((instance->G1_machine_V_MBtoSB * \
        ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X \
        * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X))))) - (instance->G1_machine_V_MBtoSB \
        ...
        * (instance->transformer_m * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X))))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
    ...
    _A3[17] = (-1.0) * ((((instance->transformer_m * instance->transformer_r * (1.0 + ((instance->L1_R * instance->L1_G) \
        - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X)))) \
        ...
        * ESP_sin(instance->G1_machine_delta))) + (((instance->transformer_m * instance->transformer_x * (1.0 + ((instance->L1_R \
        * instance->L1_G) - (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_X)))) - (instance->transformer_m * instance->transformer_r * (((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_R) - ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G))))) \
        * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta))) + ((((instance->G1_machine_V_MBtoSB * \
        ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L1_R * instance->L1_G) - (instance->L1_X \
    ...
    _A3[22] = (((((instance->transformer_m * instance->transformer_x * ((instance->L1_R * instance->L1_B) + (instance->L1_X \
        * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) - (instance->transformer_m * \
        instance->transformer_r * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta))) \
        - (((instance->transformer_m * instance->transformer_r * ((instance->L1_R * instance->L1_B) + (instance->L1_X * \
        instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) + (instance->transformer_m * \
        instance->transformer_x * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta)))) \
        + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * ((instance->L1_R \
        * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R)))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (instance->transformer_m \
        * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X)))))) * instance->G1_machine_x2q) / (ESP_square(instance->transformer_m)))) \
        - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (instance->transformer_m * ((instance->L1_R \
        * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R)))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m \
        * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X)))))) * instance->G1_machine_ra) / (ESP_square(instance->transformer_m)));
    _A3[23] = (-1.0) * ((((instance->transformer_m * instance->transformer_r * ((instance->L1_R * instance->L1_B) + (instance->L1_X \
        * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) + (instance->transformer_m * \
        instance->transformer_x * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X))))) * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta))) \
        + (((instance->transformer_m * instance->transformer_x * ((instance->L1_R * instance->L1_B) + (instance->L1_X * \
        instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R))) - (instance->transformer_m * \
        instance->transformer_r * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ...
        * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X)))))) * instance->G1_machine_x2d) / (ESP_square(instance->transformer_m))));
    _A3[24] = 0.0;
    _A3[25] = 0.0;
    _A3[26] = (-1.0) * instance->L2_R;
    _A3[27] = instance->L2_X;
    _A3[28] = ((((instance->transformer_m * instance->transformer_x * ((instance->L2_R * instance->L2_B) + (instance->L2_X \
        * instance->L2_G))) - (instance->transformer_m * instance->transformer_r * (1.0 + ((instance->L2_R * instance->L2_G) \
        - (instance->L2_X * instance->L2_B))))) * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta))) \
        + (((instance->transformer_m * instance->transformer_x * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X \
        * instance->L2_B)))) + (instance->transformer_m * instance->transformer_r * ((instance->L2_R * instance->L2_B) \
        ...
        + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L2_R \
        * instance->L2_G) - (instance->L2_X * instance->L2_B))))) + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (instance->transformer_m * ((instance->L2_R * instance->L2_B) + (instance->L2_X * instance->L2_G))))) * instance->G1_machine_x2q) \
        / (ESP_square(instance->transformer_m)))) - ((((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (instance->transformer_m * (1.0 + ((instance->L2_R * instance->L2_G) - (instance->L2_X * instance->L2_B))))) \
    ...
    _A3[30] = 0.0;
    _A3[31] = 0.0;
    _A3[32] = (-1.0) * instance->L2_X;
    _A3[33] = (-1.0) * instance->L2_R;
    _A3[34] = (((((instance->transformer_m * instance->transformer_x * ((instance->L2_R * instance->L2_B) + (instance->L2_X \
        * instance->L2_G))) - (instance->transformer_m * instance->transformer_r * (1.0 + ((instance->L2_R * instance->L2_G) \
        ...
        instance->L2_G))) + (instance->transformer_m * instance->transformer_x * (1.0 + ((instance->L2_R * instance->L2_G) \
        - (instance->L2_X * instance->L2_B))))) * (instance->G1_machine_I_MBtoSB * ESP_cos(instance->G1_machine_delta)))) \
        + ((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * ((instance->L2_R \
        ...
        * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L2_R * instance->L2_G) - \
        (instance->L2_X * instance->L2_B)))))) * instance->G1_machine_ra) / (ESP_square(instance->transformer_m)));
    _A3[35] = (-1.0) * ((((instance->transformer_m * instance->transformer_r * ((instance->L2_R * instance->L2_B) + (instance->L2_X \
        * instance->L2_G))) + (instance->transformer_m * instance->transformer_x * (1.0 + ((instance->L2_R * instance->L2_G) \
        - (instance->L2_X * instance->L2_B))))) * (instance->G1_machine_I_MBtoSB * ESP_sin(instance->G1_machine_delta))) \
    ...
    solution_buffer_for_x_6[0] = (-1.0) * ((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * ((1.0 \
        / instance->transformer_m) + (((-1.0) * instance->transformer_m) / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2d) \
        + (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) \
        * instance->transformer_m) / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2q));
    solution_buffer_for_x_6[1] = (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) \
        + (((-1.0) * instance->transformer_m) / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2d) - (instance->G1_machine_V_MBtoSB \
        * ESP_sin(instance->G1_machine_delta) * ((1.0 / instance->transformer_m) + (((-1.0) * instance->transformer_m) \
        / (ESP_square(instance->transformer_m)))) * instance->G1_machine_e2q);
    solution_buffer_for_x_6[2] = instance->B3_p_vr - (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        ...
        * ESP_cos(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L1_R * instance->L1_G) - \
        (instance->L1_X * instance->L1_B) - ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) \
        ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) * instance->L1_X))))) \
    ...
    solution_buffer_for_x_6[3] = instance->B3_p_vi - (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        * (instance->transformer_m * ((instance->L1_R * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? (-2220.0) : 0.0) * instance->L1_R)))) - (instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) \
        * (instance->transformer_m * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + \
        ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? 2220.0 : 0.0) * instance->L1_X)))))) * instance->G1_machine_e2d) \
        + (((instance->G1_machine_V_MBtoSB * ESP_cos(instance->G1_machine_delta) * (instance->transformer_m * ((instance->L1_R \
        * instance->L1_B) + (instance->L1_X * instance->L1_G) + ((ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_true)) ? (-2220.0) : 0.0) \
        * instance->L1_R)))) + (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * (instance->transformer_m \
        * (1.0 + (((instance->L1_R * instance->L1_G) - (instance->L1_X * instance->L1_B)) + ((ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), \
        ESP_true)) ? 2220.0 : 0.0) * instance->L1_X)))))) * instance->G1_machine_e2q)) / (ESP_square(instance->transformer_m)));
    solution_buffer_for_x_6[4] = instance->B3_p_vr - (((((instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) \
        ...
        * ESP_cos(instance->G1_machine_delta) * (instance->transformer_m * (1.0 + ((instance->L2_R * instance->L2_G) - \
        (instance->L2_X * instance->L2_B))))) - (instance->G1_machine_V_MBtoSB * ESP_sin(instance->G1_machine_delta) * \
    ...
    /* (441:3) if signal in SOLVE_LINEAR_EQUATIONS_FAILED then... */
    ESP_tmp_signals = instance->ErrorSignals & ErrorSignal_SOLVE_LINEAR_EQUATIONS_FAILED;
    if(ESP_tmp_signals != ErrorSignal_NONE) {
        instance->ErrorSignals ^= ESP_tmp_signals;
        /* Propagate error but leave states as they are: */

        /* (443:4) signal SOLVE_LINEAR_EQUATIONS_FAILED */
        instance->ErrorSignals |= ErrorSignal_SOLVE_LINEAR_EQUATIONS_FAILED;
    } else {
        /* Only if the system can be solved, assign computed states: */

        /* (446:4) self.'transformer.n.ii' := 'solution_buffer.for.x[6]'[1]; */
        instance->transformer_n_ii = solution_buffer_for_x_6[0];

        /* (447:4) self.'transformer.n.ir' := 'solution_buffer.for.x[6]'[2]; */
        instance->transformer_n_ir = solution_buffer_for_x_6[1];

        /* (448:4) self.'L2.p.ir' := 'solution_buffer.for.x[6]'[3]; */
        instance->L2_p_ir = solution_buffer_for_x_6[2];

        /* (449:4) self.'L2.p.ii' := 'solution_buffer.for.x[6]'[4]; */
        instance->L2_p_ii = solution_buffer_for_x_6[3];

        /* (450:4) self.'G1.machine.iq' := 'solution_buffer.for.x[6]'[5]; */
        instance->G1_machine_iq = solution_buffer_for_x_6[4];

        /* (451:4) self.'G1.machine.id' := 'solution_buffer.for.x[6]'[6]; */
        instance->G1_machine_id = solution_buffer_for_x_6[5];
    }

    /* (453:3) 'transformer.p.ir' := self.'G1.machine.I_MBtoSB' * ((sin(self.'G1.machine.delta') * self.'G1.machine.id') + (cos(self.'G1.machine.delta') * self.'G1.machine.iq')); */
    ...
    /* (454:3) 'transformer.p.ii' := self.'G1.machine.I_MBtoSB' * ((sin(self.'G1.machine.delta') * self.'G1.machine.iq') - (cos(self.'G1.machine.delta') * self.'G1.machine.id')); */
    ...
    /* (455:3) 'G1.machine.vd' := (self.'G1.machine.e2d' - (self.'G1.machine.ra' * self.'G1.machine.id')) + (self.'G1.machine.x2q' * self.'G1.machine.iq'); */
    ...
    /* (456:3) 'G1.machine.vq' := self.'G1.machine.e2q' - ((self.'G1.machine.ra' * self.'G1.machine.iq') + (self.'G1.machine.x2d' * self.'G1.machine.id')); */
    ...
    /* (457:3) 'B1.p.vr' := self.'G1.machine.V_MBtoSB' * ((sin(self.'G1.machine.delta') * 'G1.machine.vd') + (cos(self.'G1.machine.delta') * 'G1.machine.vq')); */
    B1_p_vr = instance->G1_machine_V_MBtoSB * ((ESP_sin(instance->G1_machine_delta) * G1_machine_vd) + (ESP_cos(instance->G1_machine_delta) \
        * G1_machine_vq));

    /* (458:3) 'B2.p.vr' := ((('B1.p.vr' / (self.'transformer.m' ^ 2)) - (self.'transformer.r' * 'transformer.p.ir')) + (self.'transformer.x' * 'transformer.p.ii')) * self.'transformer.m'; */
    ...
    /* (459:3) 'pwFault4efmi.p.ii' := if real(DymolaConvertInputToREAL0) == false then 0.0 elseif real(DymolaConvertInputToREAL0) == true (-2.22e+3) * 'B2.p.vr' else 0.0; */
    pwFault4efmi_p_ii = ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? ((-2220.0) * B2_p_vr) : 0.0;

    /* (460:3) 'B1.p.vi' := self.'G1.machine.V_MBtoSB' * ((sin(self.'G1.machine.delta') * 'G1.machine.vq') - (cos(self.'G1.machine.delta') * 'G1.machine.vd')); */
    B1_p_vi = instance->G1_machine_V_MBtoSB * ((ESP_sin(instance->G1_machine_delta) * G1_machine_vq) - (ESP_cos(instance->G1_machine_delta) \
        * G1_machine_vd));

    /* (461:3) 'B2.p.vi' := (('B1.p.vi' / (self.'transformer.m' ^ 2)) - ((self.'transformer.r' * 'transformer.p.ii') + (self.'transformer.x' * 'transformer.p.ir'))) * self.'transformer.m'; */
    ...
    /* (462:3) 'pwFault4efmi.p.ir' := if real(DymolaConvertInputToREAL0) == false then 0.0 elseif real(DymolaConvertInputToREAL0) == true 2.22e+3 * 'B2.p.vi' else 0.0; */
    pwFault4efmi_p_ir = ESP_isTrue(ESP_cmp_eq(instance, real(DymolaConvertInputToREAL0), ESP_false)) ? 0.0 : ESP_isTrue(ESP_cmp_eq(instance, \
        real(DymolaConvertInputToREAL0), ESP_true)) ? (2220.0 * B2_p_vi) : 0.0;

    /* (463:3) 'L1.p.ir' := (-1.0) * ((self.'L2.p.ir' + 'pwFault4efmi.p.ir') + self.'transformer.n.ir'); */
    L1_p_ir = (-1.0) * (instance->L2_p_ir + pwFault4efmi_p_ir + instance->transformer_n_ir);

    /* (464:3) 'L1.p.ii' := (-1.0) * ((self.'L2.p.ii' + 'pwFault4efmi.p.ii') + self.'transformer.n.ii'); */
    L1_p_ii = (-1.0) * (instance->L2_p_ii + pwFault4efmi_p_ii + instance->transformer_n_ii);

    /* (465:3) self.'derivative(G1.machine.e1q)' := (((1.0 - (self.'G1.machine.Taa' / self.'G1.machine.T1d0')) * 'G1.machine.vf_MB') - (self.'G1.machine.e1q' + (((self.'G1.machine.xd' - self.'G1.machine.x1d') - ((((self.'G1.machine.T2d0' * self.'G1.machine.x2d') * (self.'G1.machine.xd' - self.'G1.machine.x1d')) / self.'G1.machine.T1d0') / self.'G1.machine.x1d')) * self.'G1.machine.id'))) / self.'G1.machine.T1d0'; */
    ...
    /* (467:3) 'G1.machine.pe' := (('G1.machine.vq' + (self.'G1.machine.ra' * self.'G1.machine.iq')) * self.'G1.machine.iq') + (('G1.machine.vd' + (self.'G1.machine.ra' * self.'G1.machine.id')) * self.'G1.machine.id'); */
    ...
    /* (468:3) self.'derivative(G1.machine.w)' := ((self.'G1.machine.pm00' * self.'G1.machine.S_SBtoMB') - 'G1.machine.pe') / self.'G1.machine.M'; */
    instance->der_G1_machine_w = ((instance->G1_machine_pm00 * instance->G1_machine_S_SBtoMB) - G1_machine_pe) / instance->G1_machine_M;

    /* (469:3) self.'derivative(G1.machine.delta)' := self.'G1.machine.w_b' * (self.'G1.machine.w' - 1.0); */
    instance->der_G1_machine_delta = instance->G1_machine_w_b * (instance->G1_machine_w - 1.0);

    /* (470:3) self.'derivative(G1.machine.e1d)' := ((((self.'G1.machine.xq' - self.'G1.machine.x1q') - ((((self.'G1.machine.T2q0' * self.'G1.machine.x2q') * (self.'G1.machine.xq' - self.'G1.machine.x1q')) / self.'G1.machine.T1q0') / self.'G1.machine.x1q')) * self.'G1.machine.iq') - self.'G1.machine.e1d') / self.'G1.machine.T1q0'; */
    ...
    /* (471:3) self.'derivative(G1.machine.e2d)' := ((self.'G1.machine.e1d' - self.'G1.machine.e2d') + (((self.'G1.machine.x1q' - self.'G1.machine.x2q') + ((((self.'G1.machine.T2q0' * self.'G1.machine.x2q') * (self.'G1.machine.xq' - self.'G1.machine.x1q')) / self.'G1.machine.T1q0') / self.'G1.machine.x1q')) * self.'G1.machine.iq')) / self.'G1.machine.T2q0'; */
    ...
    /* (474:3) self.w := self.'G1.machine.w'; */
    instance->w = instance->G1_machine_w;

    /* (475:3) self.v := sqrt(('B1.p.vr' ^ 2) + ('B1.p.vi' ^ 2)); */
    instance->v = ESP_sqrt((ESP_square(B1_p_vr)) + (ESP_square(B1_p_vi)));

    /* (476:3) 'G1.P' := (('B1.p.vr' * 'transformer.p.ir') + ('B1.p.vi' * 'transformer.p.ii')) * self.'SysData.S_b'; */
    G1_P = ((B1_p_vr * transformer_p_ir) + (B1_p_vi * transformer_p_ii)) * instance->SysData_S_b;

    /* (477:3) self.Pgen := 'G1.P' / max(self.'SysData.S_b', 9.9999999999999997e-61); */
    instance->Pgen = G1_P / ESP_max(instance->SysData_S_b, 1.0E-60);

    /* (478:3) 'G1.Q' := (-1.0) * ((('B1.p.vr' * 'transformer.p.ii') - ('B1.p.vi' * 'transformer.p.ir')) * self.'SysData.S_b'); */
    G1_Q = (-1.0) * (((B1_p_vr * transformer_p_ii) - (B1_p_vi * transformer_p_ir)) * instance->SysData_S_b);

    /* (479:3) self.Qgen := 'G1.Q' / max(self.'SysData.S_b', 9.9999999999999997e-61); */
    instance->Qgen = G1_Q / ESP_max(instance->SysData_S_b, 1.0E-60);
 ...
 * init
 **********************************************************************************************************************/
void Startup_Hb0cb05f437863e9fece1a90bb931dbfeb4ac00f3_fba3e0dfa6c8985b41bcbe3594ee941ce98b740c(ALGOSTRUCT *instance) {
    instance->ErrorSignals = ErrorSignal_NONE;
    Startup(instance);
}
 ...
 * step
 **********************************************************************************************************************/
void DoStep_Hb0cb05f437863e9fece1a90bb931dbfeb4ac00f3_fba3e0dfa6c8985b41bcbe3594ee941ce98b740c(ALGOSTRUCT *instance) {
    instance->ErrorSignals = ErrorSignal_NONE;
    DoStep(instance);
}
 ...
 * recalibrate
 **********************************************************************************************************************/
void Recalibrate_Hb0cb05f437863e9fece1a90bb931dbfeb4ac00f3_fba3e0dfa6c8985b41bcbe3594ee941ce98b740c(ALGOSTRUCT *instance) \
    {
    instance->ErrorSignals = ErrorSignal_NONE;
    Recalibrate(instance);
}
 ...
 * reinitialize
 **********************************************************************************************************************/
void Reinitialize_Hb0cb05f437863e9fece1a90bb931dbfeb4ac00f3_fba3e0dfa6c8985b41bcbe3594ee941ce98b740c(ALGOSTRUCT *instance) \
    {
    instance->ErrorSignals = ErrorSignal_NONE;
    Reinitialize(instance);
}
