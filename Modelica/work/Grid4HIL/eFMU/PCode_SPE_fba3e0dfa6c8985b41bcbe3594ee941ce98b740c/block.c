...
static void DoStep(ALGOSTRUCT *instance)
{
    ...
    /* (437:3) if signal in SOLVE_LINEAR_EQUATIONS_FAILED then... */
    ESP_tmp_signals = instance->ErrorSignals & ErrorSignal_SOLVE_LINEAR_EQUATIONS_FAILED;
    if(ESP_tmp_signals != ErrorSignal_NONE) {
        instance->ErrorSignals ^= ESP_tmp_signals;
        /* Propagate error but leave states as they are: */

        /* (439:4) signal SOLVE_LINEAR_EQUATIONS_FAILED */
        instance->ErrorSignals |= ErrorSignal_SOLVE_LINEAR_EQUATIONS_FAILED;
    } else {
        /* Only if the system can be solved, assign computed states: */

        /* (442:4) self.'transformer.n.ii' := 'solution_buffer.for.x[6]'[1]; */
        instance->transformer_n_ii = solution_buffer_for_x_6[0];

        /* (443:4) self.'transformer.n.ir' := 'solution_buffer.for.x[6]'[2]; */
        instance->transformer_n_ir = solution_buffer_for_x_6[1];

        /* (444:4) self.'L2.p.ir' := 'solution_buffer.for.x[6]'[3]; */
        instance->L2_p_ir = solution_buffer_for_x_6[2];

        /* (445:4) self.'L2.p.ii' := 'solution_buffer.for.x[6]'[4]; */
        instance->L2_p_ii = solution_buffer_for_x_6[3];

        /* (446:4) self.'G1.machine.iq' := 'solution_buffer.for.x[6]'[5]; */
        instance->G1_machine_iq = solution_buffer_for_x_6[4];

        /* (447:4) self.'G1.machine.id' := 'solution_buffer.for.x[6]'[6]; */
        instance->G1_machine_id = solution_buffer_for_x_6[5];
    }

    /* (449:3) 'transformer.p.ir' := self.'G1.machine.I_MBtoSB' * ((sin(self.'G1.machine.delta') * self.'G1.machine.id') + (cos(self.'G1.machine.delta') * self.'G1.machine.iq')); */
    ...
    /* (450:3) 'transformer.p.ii' := self.'G1.machine.I_MBtoSB' * ((sin(self.'G1.machine.delta') * self.'G1.machine.iq') - (cos(self.'G1.machine.delta') * self.'G1.machine.id')); */
    ...
    /* (451:3) 'G1.machine.vd' := (self.'G1.machine.e2d' - (self.'G1.machine.ra' * self.'G1.machine.id')) + (self.'G1.machine.x2q' * self.'G1.machine.iq'); */
    ...
    /* (452:3) 'G1.machine.vq' := self.'G1.machine.e2q' - ((self.'G1.machine.ra' * self.'G1.machine.iq') + (self.'G1.machine.x2d' * self.'G1.machine.id')); */
    ...
    /* (453:3) 'B1.p.vr' := self.'G1.machine.V_MBtoSB' * ((sin(self.'G1.machine.delta') * 'G1.machine.vd') + (cos(self.'G1.machine.delta') * 'G1.machine.vq')); */
    B1_p_vr = instance->G1_machine_V_MBtoSB * ((ESP_sin(instance->G1_machine_delta) * G1_machine_vd) + (ESP_cos(instance->G1_machine_delta) \
        * G1_machine_vq));

    /* (454:3) 'B2.p.vr' := ((('B1.p.vr' / (self.'transformer.m' ^ 2)) - (self.'transformer.r' * 'transformer.p.ir')) + (self.'transformer.x' * 'transformer.p.ii')) * self.'transformer.m'; */
    ...
    /* (455:3) 'pwFault4efmi.p.ii' := if real(DymolaConvertInputToREAL0) == false then 0.0 elseif real(DymolaConvertInputToREAL0) == true (-2.22e+3) * 'B2.p.vr' else 0.0; */
    ...
    /* (456:3) 'B1.p.vi' := self.'G1.machine.V_MBtoSB' * ((sin(self.'G1.machine.delta') * 'G1.machine.vq') - (cos(self.'G1.machine.delta') * 'G1.machine.vd')); */
    B1_p_vi = instance->G1_machine_V_MBtoSB * ((ESP_sin(instance->G1_machine_delta) * G1_machine_vq) - (ESP_cos(instance->G1_machine_delta) \
        * G1_machine_vd));

    /* (457:3) 'B2.p.vi' := (('B1.p.vi' / (self.'transformer.m' ^ 2)) - ((self.'transformer.r' * 'transformer.p.ii') + (self.'transformer.x' * 'transformer.p.ir'))) * self.'transformer.m'; */
    ...
    /* (458:3) 'pwFault4efmi.p.ir' := if real(DymolaConvertInputToREAL0) == false then 0.0 elseif real(DymolaConvertInputToREAL0) == true 2.22e+3 * 'B2.p.vi' else 0.0; */
    ...
    /* (459:3) 'L1.p.ir' := (-1.0) * ((self.'L2.p.ir' + 'pwFault4efmi.p.ir') + self.'transformer.n.ir'); */
    L1_p_ir = (-1.0) * (instance->L2_p_ir + pwFault4efmi_p_ir + instance->transformer_n_ir);

    /* (460:3) 'L1.p.ii' := (-1.0) * ((self.'L2.p.ii' + 'pwFault4efmi.p.ii') + self.'transformer.n.ii'); */
    L1_p_ii = (-1.0) * (instance->L2_p_ii + pwFault4efmi_p_ii + instance->transformer_n_ii);

    /* (461:3) self.'derivative(G1.machine.e1q)' := (((1.0 - (self.'G1.machine.Taa' / self.'G1.machine.T1d0')) * 'G1.machine.vf_MB') - (self.'G1.machine.e1q' + (((self.'G1.machine.xd' - self.'G1.machine.x1d') - ((((self.'G1.machine.T2d0' * self.'G1.machine.x2d') * (self.'G1.machine.xd' - self.'G1.machine.x1d')) / self.'G1.machine.T1d0') / self.'G1.machine.x1d')) * self.'G1.machine.id'))) / self.'G1.machine.T1d0'; */
    ...
    /* (463:3) 'G1.machine.pe' := (('G1.machine.vq' + (self.'G1.machine.ra' * self.'G1.machine.iq')) * self.'G1.machine.iq') + (('G1.machine.vd' + (self.'G1.machine.ra' * self.'G1.machine.id')) * self.'G1.machine.id'); */
    ...
    /* (464:3) self.'derivative(G1.machine.w)' := ((self.'G1.machine.pm00' * self.'G1.machine.S_SBtoMB') - 'G1.machine.pe') / self.'G1.machine.M'; */
    instance->der_G1_machine_w = ((instance->G1_machine_pm00 * instance->G1_machine_S_SBtoMB) - G1_machine_pe) / instance->G1_machine_M;

    /* (465:3) self.'derivative(G1.machine.delta)' := self.'G1.machine.w_b' * (self.'G1.machine.w' - 1.0); */
    instance->der_G1_machine_delta = instance->G1_machine_w_b * (instance->G1_machine_w - 1.0);

    /* (466:3) self.'derivative(G1.machine.e1d)' := ((((self.'G1.machine.xq' - self.'G1.machine.x1q') - ((((self.'G1.machine.T2q0' * self.'G1.machine.x2q') * (self.'G1.machine.xq' - self.'G1.machine.x1q')) / self.'G1.machine.T1q0') / self.'G1.machine.x1q')) * self.'G1.machine.iq') - self.'G1.machine.e1d') / self.'G1.machine.T1q0'; */
    ...
    /* (467:3) self.'derivative(G1.machine.e2d)' := ((self.'G1.machine.e1d' - self.'G1.machine.e2d') + (((self.'G1.machine.x1q' - self.'G1.machine.x2q') + ((((self.'G1.machine.T2q0' * self.'G1.machine.x2q') * (self.'G1.machine.xq' - self.'G1.machine.x1q')) / self.'G1.machine.T1q0') / self.'G1.machine.x1q')) * self.'G1.machine.iq')) / self.'G1.machine.T2q0'; */
    ...
    /* (470:3) self.w := self.'G1.machine.w'; */
    instance->w = instance->G1_machine_w;

    /* (471:3) self.v := sqrt(('B1.p.vr' ^ 2) + ('B1.p.vi' ^ 2)); */
    instance->v = ESP_sqrt((ESP_square(B1_p_vr)) + (ESP_square(B1_p_vi)));

    /* (472:3) self.Pgen := (('B1.p.vr' * 'transformer.p.ir') + ('B1.p.vi' * 'transformer.p.ii')) * self.'SysData.S_b'; */
    instance->Pgen = ((B1_p_vr * transformer_p_ir) + (B1_p_vi * transformer_p_ii)) * instance->SysData_S_b;

    /* (473:3) self.Qgen := (-1.0) * ((('B1.p.vr' * 'transformer.p.ii') - ('B1.p.vi' * 'transformer.p.ir')) * self.'SysData.S_b'); */
    instance->Qgen = (-1.0) * (((B1_p_vr * transformer_p_ii) - (B1_p_vi * transformer_p_ir)) * instance->SysData_S_b);
