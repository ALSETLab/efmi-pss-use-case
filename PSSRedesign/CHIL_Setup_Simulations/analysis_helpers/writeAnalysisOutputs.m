...
for k = 1:numel(analysis.comparisons)
    comp = analysis.comparisons(k);
    if ~comp.ok
        row = sprintf('\\texttt{%s} & --- & --- & ---', comp.paperLabel);
        lines{end+1} = [row, ' \\']; %#ok<AGROW>
    else
        row = sprintf('\\texttt{%s} & %.4e & %.4e & %.4f', comp.paperLabel, comp.maxAbsErr, comp.rmsErr, comp.peakTime);
        lines{end+1} = [row, ' \\']; %#ok<AGROW>
    end
end

lines = [lines, {'\hline', '\end{tabular}', '\end{table}', '% ----------------------------------------------------'}];
