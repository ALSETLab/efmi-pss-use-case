...
rem Parse .gitignore exceptions and write openModel commands [cite: 1]
rem Stop parsing when this marker is reached:
rem   # Keep Grid4CHIL batch simulation artifacts generated by
for /f "usebackq delims=" %%L in ("%repoRoot%\.gitignore") do (
	if "%%L"=="# Keep Grid4CHIL batch simulation artifacts generated by" goto :afterGitignoreParse

	echo %%L| findstr /b "!Modelica/work/" >nul
	if not errorlevel 1 (
		for /f "tokens=* delims=!" %%F in ("%%L") do (
