...
/* See also matrixop.h */
#define DYNDiagnosticBufL 10000
#define DYNDiagnosticStrL "%.4000s"
#else
#define DYNDiagnosticBufL 5000
#define DYNDiagnosticStrL "%.2400s"
#endif
...
#ifdef _WIN32
#define DymolaWin32 1
#else
#define DymolaWin32 0
#endif
#ifndef beginParallelSection
#define beginParallelSection(layer, section)
#endif
#ifdef DYNCallStackData
#define DYNEXTRABEGIN3 struct DYNFunctionData_ dynFuncData, *DYNStackData_;dynFuncData=DYNGetFunctionData();DYNStackData_ = &dynFuncData;
#else 
...
#endif
static DYN_UNUSED_ATTR void DYNMergeBasic(struct BasicDDymosimStruct*DYNBO, const struct BasicDDymosimStruct*DYNB) {
	if (DYNBO && DYNB) {
...
DYMOLA_STATIC void ModelicaVFormatError(const char *string, va_list ap) {
	...
	_vsnprintf(buf,sizeof(buf)/sizeof(*buf)-1,string,ap);
...
#else
/* Code below contains workaround for bug in gcc */
#if (defined(_OPENMP) && !defined(DISABLE_DYMOLA_OPENMP))
static struct FunctionContext_ modelContext_ = { 0,0 };
#pragma omp threadprivate(modelContext_)
#else
static DYN_UNUSED_ATTR struct FunctionContext_ modelContext_ = { 0,0 };
#endif
#endif
...
#if DymolaNoGuard_
#define DYNJPushContext(a, DYNStackData_) int oldFunctionExternal=*(DYNStackData_->m_external);
#define DYNJPopContext(DYNStackData_) *(DYNStackData_->m_external)=oldFunctionExternal;
#define DYNJPushModelContext(b,a,DYNStackData_) (*(DYNStackData_->m_external)=0),
#define DYNJPopModelContext(DYNStackData_) (*(DYNStackData_->m_external)=0)
#else
#define DYNJPushContext(a, DYNStackData_) int oldFunctionExternal=*(DYNStackData_->m_external);struct FunctionContext_ alpha_={a,*(DYNStackData_->m_global)};*(DYNStackData_->m_global)=&alpha_;
#define DYNJPopContext(DYNStackData_) *(DYNStackData_->m_global)=alpha_.next;*(DYNStackData_->m_external)=oldFunctionExternal;
#define DYNJPushModelContext(b,a,DYNStackData_) (DYNStackData_->m_context->text=a,*(DYNStackData_->m_global)=DYNStackData_->m_context,*(DYNStackData_->m_external)=0),
#define DYNJPopModelContext(DYNStackData_) (*(DYNStackData_->m_global)=(void*)0,*(DYNStackData_->m_external)=0)
#endif
#endif

#if defined(DYNCallStackData)
#define DYNPCSDGlobal_jmp_buf (*(DYNStackData_->m_jmp_buf_env))
#else
#define DYNPCSDGlobal_jmp_buf DYNGlobal_jmp_buf_env
#endif
...
#if (defined(_OPENMP) && !defined(DISABLE_DYMOLA_OPENMP))
#pragma omp critical(DymosimMessages) 
#endif
	{
#ifdef DYMOSIM 
#ifdef LIBDS_DLL
...
DYMOLA_STATIC integer Dymola_Simulation_IsVariableStep_(const struct BasicIDymosimStruct*);
#define Dymola_Simulation_IsVariableStep Dymola_Simulation_IsVariableStep_((const struct BasicIDymosimStruct*)  (DSE_STRUCT iuser_))
...
static struct DYNInstanceData* DymosimGetDid();
#define homotopy(a,b) (DYNhomotopy(a,b,DymosimGetDid()))
...
/* Avoid nested critical */
#if (defined(_OPENMP) && !defined(DISABLE_DYMOLA_OPENMP)) && !(!defined(DynSimStruct) && !defined(DYMOSIM) && !defined(LIBDS_DLL) && !defined(BUILDFMU))
#pragma omp critical(DymosimMessages) 
#endif
	{
		...
		char textline[200];
		sprintf(textline, "%.160s %.16G", message, d);
		fmu_logger(textline, 1, severity);
#elif defined(Matlab6) && !defined(NO_FILE) && defined(ssPrintf)
		ssPrintf("%s %.16G\n", message, d);
#elif defined(DYM2DS)
		d2ds_print1("%s %.16G\n", message, d);
...
DYMOLA_STATIC double DymosimErrorLev(const char* message, int lev) {
  if (!(getBasicIDymosimStruct() && getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
	DymosimMessageDoubleSev(lev,(lev==1)?"Warning: The following was detected at time:" :"Warning: The following error was detected at time:", DymosimGetTime2());
...
/* gcc 4.8.3 (at least) does not allow the noreturn attribute (MODELICA_NORETURNATTR) in function definitions, so add explicit declaration first to be able to use it */
MODELICA_NORETURN DYMOLA_STATIC void DymosimErr(const char* message) MODELICA_NORETURNATTR;
MODELICA_NORETURN DYMOLA_STATIC void DymosimErr(const char* message) {
	DymosimErrorLev(message, 2);
	exit(1); /* Should not be reached */
}
double DymosimError(const char* message) {
	DymosimErr(message);
	return 0.0; /* Should not be reached */
}
DYMOLA_STATIC void AssertModelicaF(int b,const char*bs,const char*x) {
	if (!b) {
		char buf[DYNDiagnosticBufL];
		sprintf(buf, "  " DYNDiagnosticStrL "\n  Failed condition: " DYNDiagnosticStrL, x, bs);
		DymosimErr(buf);  /* should not return. */
	}
}
DYMOLA_STATIC void AssertModelicaF4(int b,const char*bs,const char*x,int lev,int*hi) {
	if (!b) {
		...
		if (lev==1) {
			...
			sprintf(buf, "  " DYNDiagnosticStrL "\n  Failed condition: " DYNDiagnosticStrL "\n", x, bs);
...
static DYN_UNUSED_ATTR double LogRelation(char *op, char *xexpr, char *yexpr, double x, double y,
                          Dymola_bool val, double t) {
/*  Example: LogRelation(">", #x, #y, x, y, (x) > (y), Time) */
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  char *boolval;
  boolval = "false";
  if (val) 
    boolval = "true";

  sprintf(buf, "  Relation " DYNDiagnosticStr " %s " DYNDiagnosticStr " (%g %s %g) became %s at time = %g",
...
#endif
	{
		DynLogEvents(action, time, event_type, expression, iresult, result);
	}
}



static DYN_UNUSED_ATTR double VariableChanged(char *var, double val, double t) {
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  sprintf(buf, "  Variable " DYNDiagnosticStr " = %g at time %g", var, val, t);
  DymosimMessage(buf);
#elif defined(RTLAB)
...
#endif
  DynLogEvents2(-1, t, 4, var, 2, val);
  return 1.0;
}
MODELICA_NORETURN static void diverr(const char* xexpr, double x, const char* yexpr, double y) MODELICA_NORETURNATTR;
MODELICA_NORETURN static void diverr(const char *xexpr, double x, const char *yexpr, double y) {
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  sprintf(buf, "\nModel error - division by zero: (" DYNDiagnosticStr ") / (" DYNDiagnosticStr ") = (%g) / (%g)\n", xexpr, yexpr, x, y);
  DymosimErr(buf);
#elif defined(BUILDFMU)
	char textline[1000];
  ...
  DymosimErr("Model error - division by zero");
...
MODELICA_NORETURN static void divinverr(const char* yexpr, double y) MODELICA_NORETURNATTR;
MODELICA_NORETURN static void divinverr(const char *yexpr, double y) {
	diverr("1.0", 1.0, yexpr, y);
}
...
#else
MODELICA_NORETURN static void GuardedError(const char* xexpr, double x, const char* op) MODELICA_NORETURNATTR;
MODELICA_NORETURN static void GuardedError(const char*xexpr, double x,const char*op) {
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  sprintf(buf, "\nModel error - %s (" DYNDiagnosticStr ") = %s (%g)\n", op,xexpr,op,x);
  DymosimErr(buf);
#elif defined(RTLAB)
  if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
	  OpalPrint("\nModel error - %s (" DYNDiagnosticStr ") = %s (%g)\n", op,xexpr,op,x);
  }
  DymosimErr("Model error - illegal value");
#elif defined(Matlab6) && !defined(NO_FILE) && defined(ssPrintf)
  if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
    ssPrintf("\nModel error - %s (" DYNDiagnosticStr ") = %s (%g)\n", op,xexpr,op,x);
  }
...
static DYN_UNUSED_ATTR double remainderDymola(double x,double y) {return x-divideDymola(x,y)*y;}
static DYN_UNUSED_ATTR double modulusDymola(double x,double y) {return x-floor(x/y)*y;}

MODELICA_NORETURN static void powerr(const char* xexpr, double x, const char* yexpr, double y) MODELICA_NORETURNATTR;
MODELICA_NORETURN static void powerr(const char *xexpr, double x, const char *yexpr, double y) {
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  sprintf(buf, "\nModel error - power: (" DYNDiagnosticStr ") ^ (" DYNDiagnosticStr ") = (%g) ^ (%g)\n", xexpr, yexpr, x, y);
  DymosimErr(buf);
#elif defined(RTLAB)
  if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
	  OpalPrint("\nModel error - power: (%s) ^ (%s) = (%g) ^ (%g)\n", xexpr, yexpr, x, y);
  }
  DymosimErr("Model error - power");
#elif defined(Matlab6) && !defined(NO_FILE) && defined(ssPrintf)
  if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
    ssPrintf("\nModel error - power: (%s) ^ (%s) = (%g) ^ (%g)\n", xexpr, yexpr, x, y);
  }
...
static double nthRootUnguarded(double x, int y) {
	if (x < 0) {
		return -pow(-x, 1.0 / y);
	} else {
		return pow(x, 1.0 / y);
...
static double nthRootGuarded(double x, const char* sx, int y, const char* sy) {
	if (y <= 0 || ((y % 2) == 0 && x < 0)) powerr(sx, x, sy, 1.0 / y);
	return nthRootUnguarded(x, y);
}

static double powGuarded(double x, const char* sx, double y, const char* sy)
{
	if ((x == 0.0 && y <= 0.0) || (x < 0.0 && floor(y) != y)) powerr(sx, x, sy, y);
	return pow(x, y);
}
static DYN_UNUSED_ATTR double powGuarded2(double x, const char* sx, int y, const char* sy)
{
	if (x == 0.0 && y < 0) powerr(sx, x, sy, y);
	return pow(x, y);
}
static DYN_UNUSED_ATTR double powUnguarded(double x, int y) {
	return pow(x, y);
}
...
static DYN_UNUSED_ATTR double RandomNormal(double time)
{
  int i;
  double r = -6;
...
static DYN_UNUSED_ATTR double log_variable(char *xname, double t, double x) {
#if !defined(DynSimStruct)
  char buf[DYNDiagnosticBuf];
  sprintf(buf, DYNDiagnosticStr "(%g) = %.16g", xname, t, x);
  DymosimMessage(buf);
#elif defined(RTLAB)
  OpalPrint("%s(%g) = %.16g\n", xname, t, x);
#elif defined(Matlab6) && !defined(NO_FILE) && defined(ssPrintf)
...
DYMOLA_STATIC double PackShape(const char * shapename)
{
...
LIBDS_API Dymola_bool handleevent6S(const char*rele,const char*sube,Dymola_bool*ql,double*qp,double*qn,double eps,double qrel,double*qz,Dymola_bool init,Dymola_bool ev,int idemand,int printevent,Dymola_bool*anyevent, Dymola_bool* anyevent2, Dymola_bool ltz,Dymola_bool invres,Dymola_bool*eventiter,double largeeps,Dymola_bool slow,double t,int,int index,int usingCVodeGodess);

#define HandleEvent2S(cross,xexpr,rel,yexpr,index,ltz,invres) \
	(DYNX(QRel_,index-1)=(cross),\
	(handleevent4Sfunc(did_,xexpr " " rel " " yexpr,"(" xexpr ") - (" yexpr ")",(DSE_STRUCT QZ_),index,*(DSE_STRUCT idemand_),DYNTime,PrintEvent,ltz,invres,usingCVodeGodess_)))

static DYN_UNUSED_ATTR double round0(double x) {
...
static DYN_UNUSED_ATTR void roundEvent4E(const char*expr, double(*func)(double), double qrel, double*res,
	double*fuzz, double epsil,
	int posi, int negi, int reli,
...
#define whenModelicaNew(cond,index) \
...
#define InitwhenModelica(cond,index) \
   {Init_=2; DYNX(QEvaluateNew_,index) = DYNX(QEvaluate_,index) = !(cond); Init_=1;}

#define whenModelica(cond,index) \
   if (Init_)  {InitwhenModelica(cond, index)}\
   whenModelicaNew(cond, index)
...
#define InitvectorWhenWORelease(cond,index,nrIndex) \
...
#if defined(DYNCallStackData)
#define DYNJvectorWhenNew(cond,index,nrIndex, DYNStackData_) \
   if (EnforceWhen_ || VectorWhenHandle(cond,DYNJIntegerTemporaryDenseOrig(QEvaluate_+index,DYNStackData_,1,nrIndex),DYNJIntegerTemporaryDenseOrig(QEvaluateNew_+index,DYNStackData_,1,nrIndex),DYNEvent,&AnyEvent_)) {\
	if (!haveDoneWhen_) {haveDoneWhen_=1;

#define DYNJvectorWhen(cond,index,nrIndex, DYNStackData_) \
   if (Init_) {InitvectorWhenWORelease(cond,index,nrIndex) DYNJRelease(DYNStackData_);} \
   DYNJvectorWhenNew(cond,index,nrIndex,DYNStackData_)

#define DYNJvectorWhenOld(cond,index,nrIndex, DYNStackData_) \
   if (Init_) {\
    int i_;\
...
#define vectorWhenNew(cond,index,nrIndex) \
...
#define vectorWhen(cond,index,nrIndex) \
   if (Init_) {InitvectorWhenWORelease(cond,index,nrIndex) Release();} \
   vectorWhenNew(cond,index,nrIndex)
...
#define StartWhenInitBlock \
	if (final_ == 2) { \
		const int Init_temp = Init_; \
		int idemand_var = 0; \
		int *idemand_ = &idemand_var;
#define EndWhenInitBlock \
		Init_ = Init_temp; \
		PopAllMarks(); \
	}

#define UpdateQEvaluate(whenindex)\
	if (DYNEvent&&(final_ ? (DymolaOneIteration_==0||DymolaOneIteration_==4) : ((DymolaOneIteration_==3||DymolaOneIteration_==4)))) {int i_;for (i_ = 0; i_ < whenindex;i_++) {QEvaluate_[i_]=QEvaluateNew_[i_];}}\
	if (DYNEvent&&!final_) {int i_;for (i_ = 0; i_ < whenindex;i_++) {QEvaluateNew_[i_]=QEvaluate_[i_];}}\

#define UpdateSampleCounters(sampleindex) \
	if (DYNEvent&&( final_ ? (DymolaOneIteration_==0||DymolaOneIteration_==4) : (DymolaOneIteration_==3||DymolaOneIteration_==4))) {int i_;for (i_=0;i_<sampleindex;i_++) {NextSampleTime_[i_]=NextSampleTimeNew_[i_];NextSampleAct_[i_]=NextSampleActNew_[i_];}}
...
#define BreakSectionURDef(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr, double mixedModeTheta, RealArray oldVector);

#define BreakSectionURStart(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr, double mixedModeTheta, RealArray oldVector) {double*oldVector__internal;oldVector__internal=oldVector.data;

#define BreakSectionURCall(nr) \
   { BreakSectionURDef(nr) switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,mixedModeTheta, oldVector)) { case 1:goto finish;case 2:goto leave;default:;} \
	}

#define BreakSectionURCallNew(nr) \
   { switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
...
#define BreakSectionUREnd() \
	   return 0;finish: return 1;leave:return 2;}

#define BreakSectionTMDef(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,double*Qold_,int thetaIter,double*mixedModeThetaP,double*mixedModeTheta2P,double*deltaTheta2P,int*thetaNrP);

#define BreakSectionTMStart(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,double*Qold_,int thetaIter,double*mixedModeThetaP,double*mixedModeTheta2P,double*deltaTheta2P,int*thetaNrP) {double mixedModeTheta,mixedModeTheta2,deltaTheta2;int thetaNr;\
mixedModeTheta=*mixedModeThetaP;mixedModeTheta2=*mixedModeTheta2P;deltaTheta2=*deltaTheta2P;thetaNr=*thetaNrP;

#define BreakSectionTMCall(nr) \
   { BreakSectionTMDef(nr) switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,Qold_,thetaIter,&mixedModeTheta,&mixedModeTheta2,&deltaTheta2,&thetaNr)) { case 1:goto finish;case 2:goto leave;default:;} \
	}
#define BreakSectionTMCallNew(nr) \
   { switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
...
#define BreakSectionTMEnd() \
	   *mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2;*thetaNrP=thetaNr;return 0;finish:*mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2;*thetaNrP=thetaNr; return 1;leave:*mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2;*thetaNrP=thetaNr;return 2;}
...
#define BreakSectionResPCallNewName(nr, name) { if (0==DYNParallelGoto) switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,name,name##__internal)) { case 1:DYNParallelGoto=1;case 2:DYNParallelGoto=2;default:;} \
	}
...
#define BreakSectionDenseJacobianData(name, type) type name##__m
#define BreakSectionSparseJacobianData(name, type) type name##__rows__internal, type name##__cols__internal

#define BreakSectionJacCommonDef(nr) \
	DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,double*Jacobian____internal,

#define BreakSectionJacDef(nr) BreakSectionJacCommonDef(nr) BreakSectionDenseJacobianData(Jacobian__, int) );
#define BreakSectionJacSparseDef(nr) BreakSectionJacCommonDef(nr) BreakSectionSparseJacobianData(Jacobian__, int*) );

#define BreakSectionJacCommonCallNewName(nr,name,data) \
   { switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,name##__internal, data)) { case 1:goto finish;case 2:goto leave;default:;} \
	}

#define BreakSectionJacCallNewName(nr,name) \
	BreakSectionJacCommonCallNewName(nr,name,BreakSectionDenseJacobianData(name, ))
#define BreakSectionJacSparseCallNewName(nr,name) \
	BreakSectionJacCommonCallNewName(nr,name,BreakSectionSparseJacobianData(name, ))

#define BreakSectionJacCommonPCallNewName(nr,name,data) \
   { if (0==DYNParallelGoto) switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,name##__internal, data)) { case 1:DYNParallelGoto=1;case 2:DYNParallelGoto=2;default:;} \
	}

#define BreakSectionJacPCallNewName(nr, name) \
	BreakSectionJacCommonPCallNewName(nr,name,BreakSectionDenseJacobianData(name, ))
#define BreakSectionJacSparsePCallNewName(nr, name) \
	BreakSectionJacCommonPCallNewName(nr,name,BreakSectionSparseJacobianData(name, ))

#define BreakSectionJacCallNew(nr) BreakSectionJacCallNewName(nr, Jacobian__)
#define BreakSectionJacSparseCallNew(nr) BreakSectionJacSparseCallNewName(nr, Jacobian__)
...
#define BreakSectionJacEnd() \
	   return 0;finish: DYN_UNUSED_LABEL_ATTR return 1;leave:DYN_UNUSED_LABEL_ATTR return 2;}
#define BreakSectionJacSparseEnd() BreakSectionJacEnd()

#define BreakSectionJacCommonStart(nr) \
	   DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
...
#define BreakSectionJacStart(nr) BreakSectionJacCommonStart(nr) BreakSectionDenseJacobianData(Jacobian__, int) ) {
#define BreakSectionJacSparseStart(nr) BreakSectionJacCommonStart(nr) BreakSectionSparseJacobianData(Jacobian__, int*) ) {
...
#define EndEqBlock (did_->eqRememberReplaceOldDynamics) = 0;\
				   ...
				   if (DYNKeepCVodeFEvalNonlinearGuesses_ && usingCVodeGodess_ && !solverHandleEq_ && (DYNIdemand2 <= 2 || DYNIdemand2 >= 5)) {\
				      int i_;\
					  EqRemember1Time_ = *time; EqRemAcc1Time_ = *time;\
					  for (i_ = 0; i_ < (int) (SizeEq_); ++i_)\
						 EqRemember1_[i_] = EqRemember2_[i_];}\
				   EndEqBlockInner
...
#ifndef DYNEventAccuracy1
#define DYNEventAccuracy1 6*DBL_EPSILON
/* Previously 4e-14 */
#endif
...
DYMOLA_STATIC int DYNTimeEvent2(double y,int less, struct BasicDDymosimStruct*duser, double time, double origTimeError, Dymola_bool*AnyEvent, Dymola_bool*AnyDEvent,int pr,int DymolaOneIteration, Dymola_bool*qt) {
	...
	if (time < y - DYNEventAccuracy1 *(fabs(time) + origTimeError)) {
...
#define ResetIfNewSimulation(v,val,oldResetCnt_) \
	...
	static DYN_UNUSED_ATTR void CheckContinuousIfCode(int c,const char*ys,double*v,double newValue) {
#ifdef INLINE_INTEGRATION
		if (c && fabs(*v-newValue)>1e1*(1+fabs(newValue)+fabs(*v))) {
#else
		if (c && fabs(*v-newValue)>1e-3*(1+fabs(newValue)+fabs(*v))) {
#endif
#if !defined(DynSimStruct)
			char buf[DYNDiagnosticBuf];
			sprintf(buf, "\nModel error - differentiated if-then-else was not continuous:\n " DYNDiagnosticStr "\n Value jumped from %g to %g.\nThere is support for using impulses to handle this; please refer to the manual.\n", ys, *v, newValue);
			DymosimErr(buf);
#elif defined(Matlab6) && !defined(NO_FILE) && defined(ssPrintf)
              if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
	            ssPrintf("\nModel error - differentiated if-then-else was not continuous:\n %s\n Value jumped from %g to %g.\n", ys, *v, newValue);
			  }
			DymosimErr("Model error - differentiated if-then-else was not continuous");
#else
            if (!(getBasicIDymosimStruct()->mPrintEvent&(1<<10))) {
			  printf("\nModel error - differentiated if-then-else was not continuous:\n %s\n Value jumped from %g to %g.\n", ys, *v, newValue);
			}
...
#define CheckContinuousIf(c,ys,nr)\
	(CheckContinuousIfCode(DYNEvent&&Iter>0&&!Init_&&DYNTime!=InitTime,ys,QCheckIf_+nr,c))
#endif
...
__declspec(dllexport) 
	...
	a.GetAdditionalFlags=GetAdditionalFlags;
	a.GetAdditionalReals = GetAdditionalReals;
	a.arrayInternalValueReference = 0;
	a.arraySizes = 0;
#ifdef GODESS
	{
...
// TODO: remove below when VS versions < VS 2015 can be safely neglected also for external resources
// This hack was re-inserted 2024-04-09 to support external resources, such as ExternData, being built with old VS versions.
// Enabled with Advanced.Translation.LinkWithIobFuncStub = true.
